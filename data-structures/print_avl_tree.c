...
node_t *make_empty(node_t *t)
{
  if (t != NULL)
  {
    make_empty(t->child[LEFT]);
    make_empty(t->child[RIGHT]);
    free(t);
  }
...
node_t *find_min(node_t *t)
{
  ...
  else if (t->child[LEFT] == NULL)
  {
    return t;
  }
  else
  {
    return find_min(t->child[LEFT]);
  }
}
...
node_t *find_max(node_t *t)
{
  ...
  else if (t->child[RIGHT] == NULL)
  {
    return t;
  }
  else
  {
    return find_max(t->child[RIGHT]);
  }
}
...
node_t *find(int elem, node_t *t)
{
  ...
  if (elem < *(int*)t->data)
  {
    return find(elem, t->child[LEFT]);
  }
  else if (elem > *(int*)t->data)
  {
    return find(elem, t->child[RIGHT]);
  }
  else
  {
    return t;
...
node_t * insert(int value, node_t * t)
{
  ...
  if (value < *(int*)t->data)
  {
    t->child[LEFT] = insert(value, t->child[LEFT]);
  }
  else if (value > *(int*)t->data)
  {
      t->child[RIGHT] = insert(value, t->child[RIGHT]);
  }
  else
  {
/*    duplicate, ignore it*/
...
/*  Tree * x;*/
  ...
  if (value < *(int*)t->data)
  {
    t->child[LEFT] = delete(value, t->child[LEFT]);
  }
  else if (value > *(int*)t->data)
  {
      t->child[RIGHT] = delete(value, t->child[RIGHT]);
  }
  else if (t->child[LEFT] && t->child[RIGHT])
  {
    tmp_cell = find_min(t->child[RIGHT]);
    *(int*)t->data = *(int*)tmp_cell->data;
    t->child[RIGHT] = delete(*(int*)t->data, t->child[RIGHT]);
  }
  else
  {
    tmp_cell = t;
...
asciinode * build_ascii_tree_recursive(node_t * t)
{
  ...
  node->left = build_ascii_tree_recursive(t->child[LEFT]);
  node->right = build_ascii_tree_recursive(t->child[RIGHT]);
...
/*//child have been computed for this tree.*/
...
/*//height child of the specified tree*/
